...
export type ProfilePicturePlaneConfig = {
  height: number;
  maxHeight: number;
  marginTop: number;
  name: {
...
type ProfilePicturePlaneData = Record<string, unknown> & {
  profilePicture?: {
    data?: {
      attributes?: {
        url?: string;
...
export type ProfilePicturePlaneProps = VisualBaseProps & {
  namePlane: ProfilePicturePlaneConfig;
  data: ProfilePicturePlaneData;
};
...
export class ProfilePicturePlane extends CanvasPlane {
  ...
  planeConfig:ProfilePicturePlaneConfig;

  declare props: ProfilePicturePlaneProps;
  ...
  async createInitalCanvasContent() {
    ...
    let name:string = this.planeConfig.name?.template || '';
    name = name.replace(/\$\{([^}]+)\}/g, (match, p1) => {
      const value = getValueByPath(this.props.data, p1);
      return value !== undefined && value !== null ? String(value) : '';
    });

    let tagLine:string = this.planeConfig.tagLine?.template || '';
    ...
    // image
    const imageUrl = this.props.data.profilePicture?.data?.attributes?.url || this.props.data.profilePicture?.data?.url || this.props.data.profilePicture?.url;
    this.imageLoad = loadImage(getExpressURLIfNeeded(imageUrl));

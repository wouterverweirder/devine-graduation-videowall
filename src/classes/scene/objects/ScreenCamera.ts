import * as THREE from 'three';
import { SceneObject, SceneObjectProps } from "./SceneObject";

export type ScreenCameraProps = SceneObjectProps & {
  left: number;
  right: number;
  top: number;
  bottom: number;
}

export type OptionalScreenCameraProps = {
  [K in keyof ScreenCameraProps]?: ScreenCameraProps[K];
} & {
  [key: string]: unknown; // Allow additional properties
};

class ScreenCamera extends SceneObject {

  declare props: ScreenCameraProps;

  constructor(id = THREE.MathUtils.generateUUID(), props:OptionalScreenCameraProps = {}) {
    super(id, {
      name: 'screen',
      layers: false,
      position: {
        x: 0,
        y: 0,
        z: 0
      },
      rotation: {
        x: 0,
        y: 0,
        z: 0
      },
      left: 0,
      right: 0,
      top: 0,
      bottom: 0,
      ...props
    });
    this.type = 'camera';
  }

  get object3D() {
    return this._object3D as THREE.OrthographicCamera;
  }

  async createObject3D() {
    const near = 0;
    const far = 2;
    return new THREE.OrthographicCamera( this.props.left, this.props.right, this.props.top, this.props.bottom, near, far );
  }

  applyProps(newProps:ScreenCameraProps) {
    if (newProps.left) {
      this.object3D.left = this.props.left = newProps.left;
    }
    if (newProps.right) {
      this.object3D.right = this.props.right = newProps.right;
    }
    if (newProps.top) {
      this.object3D.top = this.props.top = newProps.top;
    }
    if (newProps.bottom) {
      this.object3D.bottom = this.props.bottom = newProps.bottom;
    }
    this.object3D.updateProjectionMatrix();
    super.applyProps(newProps);
  }
}

export { ScreenCamera }
...
export type ProjectTextPlaneConfig = {
  marginTop?: number;
...
export type ProjectTextPlaneProps = VisualBaseProps & {
  planeConfig: ProjectTextPlaneConfig;
}
...
export class ProjectTextPlane extends CanvasPlane {
  ...
  declare props: ProjectTextPlaneProps;
  ...
  async createInitalCanvasContent() {
    ...
    const gradientTop = new OffscreenCanvas(this.props.textureSize.x, (planeConfig.gradientTop?.height || 50));
    {
      ...
      if (planeConfig.gradientTop?.stops) {
        for (const stopKey in planeConfig.gradientTop.stops) {
          if (Object.hasOwnProperty.call(planeConfig.gradientTop.stops, stopKey)) {
            const stop = planeConfig.gradientTop.stops[stopKey];
    ...
    let title:string = planeConfig.title?.template || 'Project Description';
    title = title.replace(/\$\{([^}]+)\}/g, (_, p1) => {
    ...
    let content:string = planeConfig.paragraphs?.template || '';

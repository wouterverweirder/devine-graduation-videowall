...
class ProjectorApplication extends Application {
  ...
  interactionTimeoutId: ReturnType<typeof setTimeout> = 0 as unknown as ReturnType<typeof setTimeout>;
  autoNextProjectTimeoutId: ReturnType<typeof setTimeout> = 0 as unknown as ReturnType<typeof setTimeout>;
  isSingleProjection = false;
  ambientAudio: HTMLAudioElement | null = null;
    ...
    if (!this.isControlledThroughWebsocket() && this.isSingleProjection) {
      ...
      nextProjectButton.addEventListener('click', () => {
        ...
        if (!Array.isArray(projects)) {
          console.error('No projects found in fetchProjectsResult', this.fetchProjectsResult);
          return;
        }
  ...
  onSceneObjectAdded = (object:SceneObject) => {
    this.scene.add(object.object3D);
  }

  onSceneObjectRemoved = (object:SceneObject) => {
